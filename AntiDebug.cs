...
namespace SecurityTools
{
    class DebugDetector
    {
        ...
        [DllImport("kernelbase.dll", SetLastError = true)]
        private static extern bool GetThreadContext(IntPtr hThread, ref Structures.CONTEXT Context);
        ...
        [DllImport("ntdll.dll", SetLastError = true)]
        private static extern uint NtQueryInformationProcess(SafeHandle hProcess, uint ProcessInfoClass, ref Structures.PROCESS_BASIC_INFORMATION ProcessInfo, uint nSize, uint ReturnLength);
        ...
        [DllImport("kernelbase.dll", SetLastError = true)]
        private static extern void GetSystemInfo(out Structures.SYSTEM_INFO lpSystemInfo);
        ...
        {
            Structures.CONTEXT Context = new Structures.CONTEXT();
        ...
        public static bool ParentProcessAntiDebug()
        {
            try
            {
                ...
                if (NtQueryInformationProcess(Process.GetCurrentProcess().SafeHandle, ProcessBasicInformation, ref PBI, (uint)Marshal.SizeOf(typeof(Structures.PROCESS_BASIC_INFORMATION)), 0) == 0)
                {
        ...
        {
            Structures.SYSTEM_INFO SysInfo = new Structures.SYSTEM_INFO();
